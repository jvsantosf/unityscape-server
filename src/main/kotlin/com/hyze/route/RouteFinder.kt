...
import com.hyze.route.collision.CollisionFlag
import com.hyze.route.collision.CollisionStrategy
import com.hyze.route.collision.CollisionStrategyType
import com.hyze.route.collision.NormalBlockFlagCollision
import com.hyze.route.reach.DefaultReachStrategy
import com.hyze.route.reach.ReachStrategy
...
fun routeEntityToEntity(entity: Entity, target: Entity, maxTurns: Int = DEFAULT_MAX_TURNS): Route {
    return PathFinder(flags = arrayOf(intArrayOf(1)), useRouteBlockerFlags = true)
        .findPath(
            entity.x, entity.y,
            target.x, target.y,
            entity.z,
            collision = NormalBlockFlagCollision(),
            objShape = -2,
            srcSize = entity.size,
...
@JvmOverloads
fun routeEntityToTile(entity: Entity, tile: Position, maxTurns: Int = DEFAULT_MAX_TURNS): Route {
    return PathFinder(flags = arrayOfNulls(2048 * 2048 * 4), useRouteBlockerFlags = true)
        .findPath(entity.x, entity.y, tile.x, tile.y, entity.z, collision = NormalBlockFlagCollision(), srcSize = entity.size, maxTurns = maxTurns)
}
