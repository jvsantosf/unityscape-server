...
public class Region {
	...
	public void addObject(WorldObject object, int plane, int localX,
	...
	public void spawnObject(WorldObject object, int plane, int localX, int localY, boolean original) {
		...
		if (original) {
			try {
				...
				System.err.println("x: " + localX + ", y: " + localY + ", z: " + plane);
			}
		} else {
	...
	public List<WorldObject> getObjects() {
		...
		for (WorldObject[][][] object : objects) {
			...
			for (WorldObject[][] element : object) {
				if (element == null) {
					continue;
				}
	...
	public WorldObject getObjectWithId(int plane, int x, int y, int id) {
		...
		for (WorldObject object : removedOriginalObjects) {
			if (object.getId() == id && object.getXInRegion() == x && object.getYInRegion() == y && object.getZ() == plane) {
				return null;
			}
		}
		for (int i = 0; i < 4; i++) {
		...
		for (WorldObject object : spawnedObjects) {
			if (object.getXInRegion() == x && object.getYInRegion() == y && object.getZ() == plane && object.getId() == id) {
				return object;
			}
		}
		return null;
	...
	public WorldObject getObjectWithId(int id, int plane) {
		...
		for (WorldObject object : spawnedObjects) {
			if (object.getId() == id && object.getZ() == plane) {
				return object;
			}
		}
		for (int x = 0; x < 64; x++) {
	...
	public void loadRegionMap() {
		...
		boolean osrs = Constants.isOSRSRegion(regionId);
		...
		if (mapContainerData != null) {
				...
				for (int x = 0; x < 64; x++) {
					for (int y = 0; y < 64; y++) {
						if ((mapSettings[plane][x][y] & 0x1) == 1) {
							...
							if (realPlane >= 0) {
								forceGetRegionMap().clip(realPlane, x, y);
							}
						}
					}
				}
			...
			for (int plane = 0; plane < 4; plane++) {
				for (int x = 0; x < 64; x++) {
					for (int y = 0; y < 64; y++) {
						forceGetRegionMap().clip(plane, x, y);
					}
				}
			}
		}
		if (landContainerData != null) {
			...
			while ((incr = landStream.readSmart2()) != 0) {
				...
				while ((incr2 = landStream.readUnsignedSmart()) != 0) {
					...
					spawnObject(new WorldObject(objectId + (osrs ? Constants.OSRS_OBJECTS_OFFSET : 0), type, rotation, localX + regionX, localY + regionY, objectPlane), objectPlane, localX, localY, true);
				}
			}
		}
		if (Constants.DEBUG && landContainerData == null && landArchiveId != -1 && MapArchiveKeys.getMapKeys(regionId) != null) {
	 ...
	 * region.
	 */
	public FloorItem getGroundItem(int id, Position tile, Player player) {
		if (groundItems == null) {
			return null;
		}
		for (FloorItem item : groundItems) {
			...
			if (item.getId() == id && tile.getX() == item.getTile().getX() && tile.getY() == item.getTile().getY() && tile.getZ() == item.getTile().getZ()) {
				return item;
			}
		}
		return null;
	...
	public boolean containsObject(int id, WorldObject object) {
		return World.getObjectWithId(new Position(object.getZ(), object.getX(),
				object.getY()), id) != null;
	}

	public static Region get(int regionId) {
		return World.getRegion(regionId, true);
	}
	...
	public DynamicRegion asDynamic() {
		if (!(this instanceof DynamicRegion))
			throw new IllegalStateException("Region isn't instanceof DynamicRegion!");
		return (DynamicRegion) this;
	}
