...
public final class ItemsContainer<T extends Item> implements Serializable {
	...
	public boolean containsAtLeastOne(int itemId) {
		int amtOf = 0;
		for (Item aData : data) {
			if (aData != null) {
	...
	public Map<Integer, Item> removeIf(Predicate<Item> predicate) {
		Map<Integer, Item> removed = Maps.newHashMap();
		for (int slot = 0; slot < data.length; slot++) {
			Item item = get(slot);
			if (item != null && predicate.test(item)) {
				set(slot, null);
				removed.put(slot, item);
			}
		}
		return removed;
	}
