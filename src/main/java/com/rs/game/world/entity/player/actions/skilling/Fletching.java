...
public class Fletching extends Action {
																																																																																																								...
																																																																																																								RED_TOPAZ_BOLTS(9188, 9141, new int[] { 9336 }, new int[] { 48 },
																																																																																																										...
																																																																																																										BARBED_BOLTS(47, 877, new int[] { 881 }, new int[] { 51 },
																																																																																																												...
																																																																																																												SAPPHIRE_BOLTS(9189, 9142, new int[] { 9337 }, new int[] { 56 },
																																																																																																														...
																																																																																																														EMERALD_BOLTS(9190, 9142, new int[] { 9338 }, new int[] { 26 },
	...
	public static boolean maxMakeQuantityTen(Item item) {
		if (item.getId() == 52 || ItemDefinitions.getItemDefinitions(item.getId()).getName().toLowerCase().contains("dart")
				|| ItemDefinitions.getItemDefinitions(item.getId()).getName().toLowerCase().contains("bolt")
				|| item.getId() == 53 || item.getId() == 314
				|| ItemDefinitions.getItemDefinitions(item.getId()).getName().toLowerCase().contains("arrow")) {
			return true;
		}
		return false;
	}
	...
	public int processWithDelay(Player player) {
		ticks--;
		int amount = maxMakeQuantityTen(new Item(fletch.getProduct()[option])) ? player.getEquipment().wearingSkillCape(Skills.FLETCHING) ? 30 : 15 : 1;
		if (fletch.getId() == 9194 || fletch.getId() == 9191) {
			if (player.getInventory().containsItem(fletch.getId(), 15) && player.getInventory().containsItem(fletch.getSelected(), 15)){
				player.getInventory().deleteItem(fletch.getId(), fletch.getProduct()[option] == 9144 ? 14 : 14);
				player.getInventory().addItem(fletch.getProduct()[option], 14);
				player.getInventory().deleteItem(fletch.getSelected(), 14);
				player.sm("You succesfully attach 15 bolts tips to the bolts!");
			}else {
				player.sm("You must have atleast 15 tips and bolts to do this.");
				return -1;
			}
		} else if (fletch.getId() == 9143 || fletch.getId() == 9192) {
			if (player.getInventory().containsItem(fletch.getId(), 15) && player.getInventory().containsItem(fletch.getSelected(), 15)){
				player.getInventory().deleteItem(fletch.getId(), fletch.getProduct()[option] == 9144 ? 14 : 14);
				player.getInventory().addItem(fletch.getProduct()[option], 14);
				player.getInventory().deleteItem(fletch.getSelected(), 14);
