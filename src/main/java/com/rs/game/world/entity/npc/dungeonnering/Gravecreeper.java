...
public class Gravecreeper extends DungeonBoss {
	...
	public Gravecreeper(int id, Position tile, DungeonManager manager, RoomReference reference) {
	...
	public boolean removeBurnedTile(Position center) {
	...
	private void processBurnAttack() {
		for (BurnTile bTile : burnedTiles) {
			...
			for (Entity t : getPossibleTargets()) {
				Player p2 = (Player) t;
				tileLoop: for (Position tile : bTile.tiles) {
					if (p2.getX() != tile.getX() || p2.getY() != tile.getY())
						continue tileLoop;
					p2.applyHit(new Hit(this, (int) com.rs.utility.Utils.random(getMaxHit() * 2), Hit.HitLook.REGULAR_DAMAGE));
	...
	public void useSpecial() {
		Position walkTo = getNearestPlinch();
		if (walkTo == null)
			return;
		...
		setForceWalk(walkTo);
		setNextForceTalk(new ForceTalk(SPECIAL_SHOUTS[specialDelay == -1 ? 2 : com.rs.utility.Utils.random(SPECIAL_SHOUTS.length)]));
						...
						WorldTasksManager.schedule(new WorldTask() {
							...
							public void run() {
								if (getManager().isDestroyed())
									return;
								setNextPosition(getManager().getTile(getReference(), 3 + com.rs.utility.Utils.random(4) * 3, 3 + com.rs.utility.Utils.random(4) * 3));
		...
		}, com.rs.utility.Utils.getDistance(this, walkTo));
	}

	public Position getNearestPlinch() {
		...
		for (int x = 0; x < plinths.length; x++) {
			for (int y = 0; y < plinths[x].length; y++) {
				...
				if (plinth == null)
					continue;
				int d = com.rs.utility.Utils.getDistance(this, plinth);
				if (d >= distance)
					continue;
				distance = d;
				p = plinth;
	...
	public void triggerPlinths() {
		...
		for (int x = 0; x < plinths.length; x++) {
			for (int y = 0; y < plinths[x].length; y++) {
				if (plinths[x][y] == null)
					continue;
				Position altarLoc = getManager().getTile(getReference(), TOMB_LOC_POS_2[y][x][0], TOMB_LOC_POS_2[y][x][1]);
				...
				if (!triggeredPlinths[x][y]) {
					...
					for (Entity t : possibleTargets) {
						if (com.rs.utility.Utils.isOnRange(t.getX(), t.getY(), t.getSize(), altarLoc.getX(), altarLoc.getY(), 1, 2))
							t.applyHit(new Hit(this, com.rs.utility.Utils.random((int) (t.getMaxHitpoints() * 0.1)) + 1, Hit.HitLook.MAGIC_DAMAGE));
					}
				}
			}
		}
	...
	public void activatePlinths() {
		for (int x = 0; x < plinths.length; x++) {
			for (int y = 0; y < plinths[x].length; y++) {
				...
				if (plinth == null)
					continue;
				if (plinths[x][y] == null && com.rs.utility.Utils.random(15) < getManager().getParty().getTeam().size() * 3) {
	...
	public void createBurnTiles(Position tile, boolean permenant) {
		burnedTiles.add(new BurnTile(tile, permenant));
	}

	public void createBurnTiles(Position tile) {
	...
	public class BurnTile {
		...
		public BurnTile(Position center, boolean permenant) {
			this.center = center;
			this.permenant = permenant;
			tiles = new Position[9];
			int index = 0;
			for (int x = -1; x < 2; x++) {
				for (int y = -1; y < 2; y++) {
