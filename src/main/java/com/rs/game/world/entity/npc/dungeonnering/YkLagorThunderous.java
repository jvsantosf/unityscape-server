...
public class YkLagorThunderous extends DungeonBoss {
	...
	public YkLagorThunderous(int id, Position tile, DungeonManager manager, RoomReference reference) {
	...
	public void processNPC() {
		...
		if (isCantInteract()) {
			...
			if (distractedMages >= 4 && com.rs.utility.Utils.random(5) == 0) {
				animate(new Animation(19564));
			...
			animate(new Animation(19565));
			setNextGraphics(new Graphics(165));
			if (com.rs.utility.Utils.random(33) == 0 && !getPossibleTargets().isEmpty()) {
				YkLagorMage mage = mysteriousMages[com.rs.utility.Utils.random(mysteriousMages.length)];
				if (mage.isUnderCombat() || mage.isDead() || mage.hasFinished())
					return;
				int index = com.rs.utility.Utils.random(QUOTES.length);
	...
	public void sendBrokenFloor() {
		for (int index = 0; index < BROKEN_FLOORS_TILES.length; index++) {
			Position tile = getManager().getTile(getReference(), BROKEN_FLOORS_TILES[index][0], BROKEN_FLOORS_TILES[index][1]);
	...
	public static boolean isBehindPillar(Player player, DungeonManager manager, RoomReference rRef) {
		for (int index = 0; index < PILLAR_SAFEZONE.length; index++) {
			Position tile = manager.getTile(rRef, PILLAR_SAFEZONE[index][0], PILLAR_SAFEZONE[index][1]);
