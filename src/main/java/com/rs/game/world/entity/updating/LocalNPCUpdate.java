...
public final class LocalNPCUpdate {
	...
	private void processInScreenNPCs(OutputStream stream,
		...
		for (Iterator<NPC> it = localNPCs.iterator(); it.hasNext();) {
			NPC n = it.next();
			if (n.isFinished() || !n.withinDistance(player, largeSceneView ? 126 : 14) || n.hasTeleported()) {
	...
	private void appendUpdateBlock(NPC n, OutputStream data, boolean added) {
		...
		if (n.getNextFacePosition() != null && n.getNextRunDirection() == -1 && n.getNextWalkDirection() == -1) {
			maskData |= 0x80;
		}
		...
		if ((n instanceof Pet && !((Pet) n).isOwner(player)) || (n instanceof Familiar && !((Familiar) n).isOwner(player))) {
			maskData |= 0x4000000;
		}
		if (maskData > 0xff)
			maskData |= 0x2;
	...
	private void applyFaceWorldTileMask(NPC n, OutputStream data) {
		data.writeShort128((n.getNextFacePosition().getX() << 1) + 1);
		data.writeShortLE128((n.getNextFacePosition().getY() << 1) + 1);
	}
