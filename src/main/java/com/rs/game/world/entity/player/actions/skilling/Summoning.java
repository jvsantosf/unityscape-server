...
import com.rs.game.world.entity.player.actions.skilling.summoning.SummoningPouch;
import com.rs.game.world.entity.player.content.SummoningScroll;
...
public class Summoning {
	...
	public static void createPouch(final Player player, int itemId, int amount) {
		...
		player.animate(new Animation(9068));
	}
	...
	public static Familiar createFamiliar(Player player, Pouches pouch) {
		try {
			return (Familiar) Class.forName("com.rs.game.world.entity.npc.familiar." + (NPCDefinitions.getNPCDefinitions(pouch.getNpcId())).name.replace(" ", "").replace("-", "")).getConstructor(Player.class, Pouches.class, Position.class, int.class, boolean.class).newInstance(player, pouch, player, -1, true);
		} catch (Throwable e) {
			return null;
		}
	}
	...
	public static void spawnFamiliar(Player player, Pouches pouch) {
		...
		if (npc == null) {
			System.out.println("Familiar failed to summon for pouch " + pouch.getPouchId() + ".");
			return;
		}
