...
public final class CombatDefinitions implements Serializable {
	...
	public static final int getXpStyle(int weaponId, int attackStyle) {
		if (weaponId != -1 && weaponId != -2) {
			...
			if (weaponName.contains("godsword") || weaponName.contains("rapier") 
					|| weaponName.contains("sword") || weaponName.contains("2h")) {
				switch (attackStyle) {
				case 0:
					return Skills.ATTACK;
				case 1:
	...
	public void refreshBonuses() {
		ArrayList<String> noAmmoWeapons = new ArrayList<String>();
		noAmmoWeapons.add("knife");
		noAmmoWeapons.add("dart");
		noAmmoWeapons.add("cock cannon");
		noAmmoWeapons.add("javelin");
		noAmmoWeapons.add("thrownaxe");
		noAmmoWeapons.add("throwing axe");
		noAmmoWeapons.add("crystal bow");
		noAmmoWeapons.add("craw's bow");
		noAmmoWeapons.add("zaryte bow");
		noAmmoWeapons.add("seren godbow");
		noAmmoWeapons.add("chinchompa");
		noAmmoWeapons.add("decimation");
		noAmmoWeapons.add("bolas");
		noAmmoWeapons.add("blowpipe");
		
		
		boolean hasNoAmmoWeapon = false;
		
		for (String s : noAmmoWeapons) {
			if (ItemDefinitions.getItemDefinitions(player.getEquipment().getWeaponId()).getName().toLowerCase().contains(s)) {
				hasNoAmmoWeapon = true;
			}
		}
		...
		for (Item item : player.getEquipment().getItems().getItems()) {
			...
			if (!(hasNoAmmoWeapon && player.getEquipment().getAmmoId() == item.getId())) {
	...
	public boolean usingPoisonWeapon() {
		String weaponName = player.getEquipment().getWeaponId() != -1 ? ItemDefinitions.forId(player.getEquipment().getWeaponId()).getName() : "";
		if (weaponName.contains("(p"))
			return true;
		return false;
	}
	
	public boolean usingVenomWeapon() {
		String weaponName = player.getEquipment().getWeaponId() != -1 ? ItemDefinitions.forId(player.getEquipment().getWeaponId()).getName() : "";
		return weaponName.contains("toxic") || weaponName.contains("swamp") || player.petManager.getNpcId() == 16122;
	}
