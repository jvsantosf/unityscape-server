...
public class FairyRing implements Serializable {
    ...
    @Getter private FairyRings lastTeleport;
        ...
        WorldTasksManager.schedule(new WorldTask() {
            ...
            public void run() {
                if (player.getX() == object.getX() && player.getY() == object.getY() || elapseTime > .3) {
                    if (object.getX() == FairyRings.HUB.x && object.getY() == FairyRings.HUB.y && object.getZ() == FairyRings.HUB.plane) {
    ...
    public void TeleportToHub() {
        WorldTasksManager.schedule(new WorldTask() {
            ...
            public void run() {
                if (steps == 0) {
                    closeInterface();
                    player.animate(new Animation(3254));
                    player.setNextGraphics(new Graphics(2670));
                } else if (steps == 2) {
                	lastTeleport = FairyRings.HUB;
                    player.setNextPosition(FairyRings.HUB.getTile());
                } else if (steps == 3) {
                    player.setNextFacePosition(new Position(player.getX(),
                    player.getY() - 1, player.getZ()));
                } else if (steps == 4) {
                    player.animate(new Animation(3255));
    ...
    public void teleport(FairyRings ring) {
    	if (ring == null) {
    		player.sendMessage("This fairy ring location doesn't exist.");
    		return;
    	}
        WorldTasksManager.schedule(new WorldTask() {
            int steps;
            ...
            public void run() {
                if (steps == 0) {
                    ...
                    player.setNextPosition(ring.getTile());
                } else if (steps == 3) {
                    player.setNextFacePosition(new Position(player.getX(), player.getY() - 1, player.getZ()));
                } else if (steps == 4) {
                    player.animate(new Animation(3255));
                    player.setNextGraphics(new Graphics(2671));
                } else if (steps >= 5) {
        ...
        WorldTasksManager.schedule(new WorldTask() {
            ...
            public void run() {
                if (steps == 0) {
                	...
                	lastTeleport = getRing();
                    player.setNextPosition(new Position(getRing()
                        .getTile()));
                } else if (steps == 3) {
                    player.setNextFacePosition(new Position(player.getX(),
                    player.getY() - 1, player.getZ()));
