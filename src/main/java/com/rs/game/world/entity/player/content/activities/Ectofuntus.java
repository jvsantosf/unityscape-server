	...
	public static void handleWorship(Player player) {
		...
		for (Item item : player.getInventory().getItems().getItems()) {
			...
			if (bone != null) {
				...
				player.animate(new Animation(1651));
		...
		switch(objectId) {
			case 5268: {
				...
				WorldTasksManager.schedule(new WorldTask() {
					@Override
					public void run() {
						player.setNextPosition(new Position(3669, 9888, 3));
					}
				}, 0);
			}
			return true;
			...
			case 5264: {
				...
				WorldTasksManager.schedule(new WorldTask() {
					@Override
					public void run() {
						player.setNextPosition(new Position(3654, 3519, 0));
					}
				}, 0);
			}
			return true;
			...
			case 9308: {
				...
				WorldTasksManager.schedule(new WorldTask() {
					@Override
					public void run() {
						player.setNextPosition(new Position(3671, 9888, 2));
					}
				}, 1);
			}
			return true;
			...
			case 9307: {
				...
				WorldTasksManager.schedule(new WorldTask() {
					@Override
					public void run() {
						player.setNextPosition(new Position(3670, 9888, 3));
					}
				}, 1);
			}
			return true;
			...
			case 5263:
				if (player.getZ() == 3)
					player.setNextPosition(new Position(3688, 9888, 2));
				if (player.getZ() == 2)
					player.setNextPosition(new Position(3675, 9887, 1));
				if (player.getZ() == 1)
					player.setNextPosition(new Position(3683, 9888, 0));
			return true;
			
			case 5262:
				if (player.getZ() == 2)
					player.setNextPosition(new Position(3692, 9888, 3));
				if (player.getZ() == 1)
					player.setNextPosition(new Position(3671, 9888, 2));
				if (player.getZ() == 0)
					player.setNextPosition(new Position(3687, 9888, 1));
			return true;
			...
			case GRINDER_OBJECT:
				if (player.boneType != -1 && !player.bonesGrinded) {
					...
					player.animate(new Animation(1648));
			...
			case BIN_OBJECT:
				...
				if (player.boneType != -1 && player.bonesGrinded) {
					...
					if (meal != null) {
						...
						player.animate(new Animation(1650));
