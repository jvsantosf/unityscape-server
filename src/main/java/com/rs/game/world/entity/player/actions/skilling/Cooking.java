...
public class Cooking extends Action {
	...
	public enum Cookables {
		...
		RAW_MEAT_SINEW(new Item(2142, 1), 1, 200, 30, new Item(2146, 1), new Item(9436, 1), false, false),
	...
	private boolean isBurned(Cookables cook, Player player) {
		...
		return Utils.random(45) <= levelsToStopBurn;
	}
	...
	public int processWithDelay(Player player) {
		...
		if (player.getSkills().getLevel(Skills.COOKING) >= cook.getBurningLvl() ? false : isBurned(cook, player) &&
				!player.getEquipment().wearingSkillCape(Skills.COOKING)) {
			...
			} else if (cook == Cookables.RAW_MONKFISH) {
				player.monkscooked++;
			} else if (cook == Cookables.RAW_SHARK) {
				player.sharkscooked++;
			} else if (cook == Cookables.RAW_MANTA_RAY) {
				player.mantascooked++;
