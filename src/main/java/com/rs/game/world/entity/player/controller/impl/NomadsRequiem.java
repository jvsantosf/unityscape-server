...
public class NomadsRequiem extends Controller {
	...
	public static final Position OUTSIDE = new Position(1895, 3177, 0);
...
/*	private void teletoDoor(Player player, int id) {
	...
	public void enterDoor(int doorIndex) {
		player.setNextPosition(new Position(getBaseX() + currentPart.doorPositions[doorIndex*2]
	...
	public void sendFirstScene() {
		...
		final NPC nomad = World.spawnNPC(8531, new Position(getBaseX() + 41, getBaseY() + 11, 0), -1, 0, true, true);
		nomad.setRun(true);
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if(stage == 0) {
					...
					player.setNextFacePosition(nomad);
					...
					Position walk = new Position(getBaseX() + 35, getBaseY() + 11, 0);
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (stage == 1) {
					...
					Position throne = new Position(getBaseX() + 16, getBaseY() + 20, 0);
					Nomad nomad =(Nomad) World.spawnNPC(8528, throne, -1, 0, true, true);
