...
public final class DominionTower implements Serializable {
	...
	private void teleportToArena(int mode) {
		player.setNextFacePosition(new Position(getBaseX() + 11, getBaseY() + 29, 0));
		player.getControlerManager().startControler("DTControler", mode);
		player.unlock();
		player.setNextPosition(new Position(getBaseX() + 10, getBaseY() + 29,
				2));
	...
	public void startFight(final NPC[] bosses) {
		for (NPC boss : bosses) {
			...
			boss.setNextFacePosition(new Position(boss.getX() - 1, boss
					.getY(), 0));
		}
		player.lock();
		player.setNextPosition(new Position(getBaseX() + 25, getBaseY() + 32,
				2));
		player.setNextFacePosition(new Position(getBaseX() + 26, getBaseY() + 32, 0));
				...
				if (count == 0) {
					...
					if (nextBossIndex != -1 && BOSSES[index].item != null)
						World.addGroundItem(BOSSES[index].item,
								new Position(getBaseX() + 26, getBaseY() + 33,
										2));
					player.closeInterfaces();
	...
	public void loss(final int mode) {
		...
		nextBossIndex = -1;
		player.lock();
		player.setNextPosition(new Position(getBaseX() + 35, getBaseY() + 31,
				2));
		player.setNextFacePosition(new Position(player.getX() + 1, player
				.getY(), 0));
	...
	public void win(int mode) {
		removeItem();
		int factor = (getBossesTotalLevel() * (mode == CLIMBER ? 100 : 10)) * (player.getDonationManager().hasRank(DonatorRanks.DRAGON) ? 2 : 1);
		progress++;
		if (mode == CLIMBER) {
		...
		player.setNextPosition(new Position(getBaseX() + 35, getBaseY() + 31,
				2));
		player.setNextFacePosition(new Position(getBaseX() + 36, getBaseY() + 31, 0));
	...
	public void destroyArena(final boolean logout, int mode) {
		Position tile = new Position(3744, 6425, 0);
		if (logout)
	...
	public NPC[] createBosses() {
		...
		for (int i = 0; i < BOSSES[getNextBossIndex()].ids.length; i++)
			bosses[i] = World
					.spawnNPC(BOSSES[getNextBossIndex()].ids[i], new Position(
