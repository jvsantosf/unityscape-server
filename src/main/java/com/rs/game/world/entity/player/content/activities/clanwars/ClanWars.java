package com.rs.game.world.entity.player.content.activities.clanwars;
...
public final class ClanWars implements Serializable {
	 ...
	 */
	private transient Position baseLocation;
		...
		if (other != null && (Boolean) other.getTemporaryAttributtes().get("accepted_war_terms") == Boolean.TRUE) {
			CoresManager.slowExecutor.submit(new Runnable() {
				...
				public void run() {
					...
					baseLocation = new Position(newCoords[0] << 3, newCoords[1] << 3, 0);
	...
	public static void enter(Player p) {
		...
		p.getInterfaceManager().sendTab(p.getInterfaceManager().hasRezizableScreen() ? 11 : 27, 265);
		...
		if (hasWar) {
			...
			if (c.firstTeam == p.getCurrentFriendChat()) {
				p.setNextPosition(c.baseLocation.transform(c.areaType.getFirstSpawnOffsetX(), c.areaType.getFirstSpawnOffsetY(), 0));
				c.firstPlayers.add(p);
				c.timer.refresh(p, true);
			} else {
				Position northEast = c.baseLocation.transform(c.areaType.getNorthEastTile().getX() - c.areaType.getSouthWestTile().getX(),
						c.areaType.getNorthEastTile().getY() - c.areaType.getSouthWestTile().getY(), 0);
				p.setNextPosition(northEast.transform(c.areaType.getSecondSpawnOffsetX(), c.areaType.getSecondSpawnOffsetY(), 0));
		...
		if (prefix == null || prefix == 0) {
			...
			p.setNextPosition(c.baseLocation.transform(c.areaType.getFirstDeathOffsetX(), c.areaType.getFirstDeathOffsetY(), 0));
		} else {
			c.timer.refresh(p, false);
			c.secondViewers.add(p);
			Position northEast = c.baseLocation.transform(c.areaType.getNorthEastTile().getX() - c.areaType.getSouthWestTile().getX(),
					c.areaType.getNorthEastTile().getY() - c.areaType.getSouthWestTile().getY(), 0);
			p.setNextPosition(northEast.transform(c.areaType.getSecondDeathOffsetX(), c.areaType.getSecondDeathOffsetY(), 0));
		}
	}
	...
	public void leave(Player p, boolean ingame) {
		...
		p.setNextPosition(new Position(2992, 9676, 0));
	...
	public void endWar() {
		...
		Position target = new Position(2992, 9676, 0);
		int firstType;
		int secondType;
		if (timer.isTimeOut()) {
			firstType = 1;
