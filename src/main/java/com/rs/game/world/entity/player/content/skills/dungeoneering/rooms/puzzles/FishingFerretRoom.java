...
import com.rs.game.world.entity.npc.dungeonnering.DungeonFishSpot;
...
public class FishingFerretRoom extends PuzzleRoom {
	...
	private Position pressurePlate;
	...
	public class Ferret extends DungeonNPC {
		...
		public Ferret(int id, Position tile, DungeonManager manager, double multiplier) {
			...
			if (getWalkSteps().isEmpty()) {
				...
				if (o != null && o.getDefinitions().name.equals("Hole")) {
					...
					WorldTasksManager.schedule(new WorldTask() {
						...
						public void run() {
							resetWalkSteps();
							setNextPosition(getSpawnPosition());
	...
	public void openRoom() {
		...
		boolean invertChunks = com.rs.utility.Utils.random(2) == 0;
		int[] cornerChunks = possibleCorners[com.rs.utility.Utils.random(possibleCorners.length)];
		...
		psuedoFishingSpot = new DungeonFishSpot(1957, manager.getRotatedTile(reference, 7, 13), manager, DungeoneeringFishing.Fish.VILE_FISH);
