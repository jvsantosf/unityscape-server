...
public class BrimstoneChest {
    ...
    private static LootTable table = new LootTable().addItems(
            ...
            //rare
            //very rare
            new WeightedItem(28658, 1, 1),
            new WeightedItem(28650, 1, 1)
                ...
                if (item.getWeight() == 1) {
                    World.sendWorldMessage(
                            "<img=23><col=F39407>" + player.getDisplayName() + "<col=F39407> Received<col=F39407> Very rare "
                                    + item.getAmount() + "x " + item.getName() + "<col=F39407> from the brimstone chest amount opened: " + player.brimestonechest,
                            false);
                }
