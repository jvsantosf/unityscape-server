...
public class NexCombat extends CombatScript {
	...
	public Position[] NO_ESCAPE_TELEPORTS = {
			new Position(2924, 5213, 0), // north
			new Position(2934, 5202, 0), // east,
			new Position(2924, 5192, 0), // south
			new Position(2913, 5202, 0),
	}; // west
		...
		if (nex.isFollowTarget()) {
			...
			if (nex.getAttacksStage() == 0) {
				...
				if (nex.getLastVirus() < Utils.currentTimeMillis() && Utils.getRandom(2) == 0) {
					...
					npc.animate(new Animation(17413));
			...
			if (Utils.getRandom(nex.getStage() == 4 ? 5 : 10) == 0) {
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						if (count == 0) {
							npc.animate(new Animation(17411));
							npc.setNextGraphics(new Graphics(1216));
						} else if (count == 1) {
							nex.setNextPosition(dir);
							...
							for (Entity entity: nex.calculatePossibleTargets(
								...
								if (entity instanceof Player) {
									...
									player.animate(new Animation(17408));
									player.setNextForceMovement(new ForceMovement(
											player, 1, idx == 3 ? 3 : idx == 2 ? 2 : idx == 1 ? 1 : 0));
								}
							}
						} else if (count == 3) {
							nex.setNextPosition(center);
						} else if (count == 4) {
			...
			if (Utils.getRandom(15) == 0) {
				...
				if (settedTarget != null && distance > 10) {
					...
					npc.animate(new Animation(17414));
					npc.setTarget(player);
					player.animate(new Animation(17408));
					WorldTasksManager.schedule(new WorldTask() {@Override
					public void run() {
						player.setNextPosition(nex);
						player.getPackets()
			...
			if (nex.getAttacksStage() == 0) {
				...
				if (!nex.isEmbracedShadow()) {
					...
					npc.animate(new Animation(17412));
				...
				if (!nex.isTrapsSettedUp() && Utils.getRandom(2) == 0) {
					...
					npc.animate(new Animation(17407));
