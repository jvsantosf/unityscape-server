...
public final class Stomp extends DungeonBoss {
	...
	public Stomp(int id, Position tile, DungeonManager manager, RoomReference reference) {
					...
					for (int[] s : shadows) {
						...
						for (Entity target : possibleTargets)
							if (target.getX() == object.getX() && target.getY() == object.getY())
								target.applyHit(new Hit(this, 1 + com.rs.utility.Utils.random((int) (target.getMaxHitpoints() * 0.8)), Hit.HitLook.REGULAR_DAMAGE));
					}
				}
				if (count == 1) {
					...
					if (lodestones[0] == true && lodestones[1] == true) {
						...
						applyHit(new Hit(null, (int) (getMaxHitpoints() * 0.25), Hit.HitLook.HEALED_DAMAGE));
	...
	public void charge() {
		...
		lodeStoneType = com.rs.utility.Utils.random(3);
		refreshLodestones();
		...
		for (int count = 0; count < 11; count++) {
			l: for (int i = 0; i < DungeonConstants.SET_RESOURCES_MAX_TRY; i++) {
				int x = 2 + com.rs.utility.Utils.getRandom(12);
				int y = 2 + com.rs.utility.Utils.getRandom(9);
