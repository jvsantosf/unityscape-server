...
public final class WarControler extends Controller {
			...
			public void run() {
				if (loop == 0) {
					...
					if (clanWars.getFirstTeam() == player.getCurrentFriendChat()) {
						player.setNextPosition(clanWars.getBaseLocation().transform(clanWars.getAreaType().getFirstDeathOffsetX(), clanWars.getAreaType().getFirstDeathOffsetY(), 0));
						...
						Position northEast = clanWars.getBaseLocation().transform(clanWars.getAreaType().getNorthEastTile().getX() - clanWars.getAreaType().getSouthWestTile().getX(),
								clanWars.getAreaType().getNorthEastTile().getY() - clanWars.getAreaType().getSouthWestTile().getY(), 0);
						player.setNextPosition(northEast.transform(clanWars.getAreaType().getSecondDeathOffsetX(), clanWars.getAreaType().getSecondDeathOffsetY(), 0));
	...
	public void moved() {
		...
		case FORSAKEN_QUARRY:
			Position northEast = clanWars.getBaseLocation().transform(clanWars.getAreaType().getNorthEastTile().getX() - clanWars.getAreaType().getSouthWestTile().getX(),
				clanWars.getAreaType().getNorthEastTile().getY() - clanWars.getAreaType().getSouthWestTile().getY(), 0)
				.transform(-16, -16, 0);
			Position southWest = clanWars.getBaseLocation().transform(16, 16, 0);
	...
	public boolean logout() {
		player.setLocation(new Position(2992, 9676, 0));
		return true;
	}
