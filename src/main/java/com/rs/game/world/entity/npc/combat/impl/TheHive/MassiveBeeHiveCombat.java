...
import com.rs.game.world.entity.npc.instances.TheHive.NestNpc;
...
public class MassiveBeeHiveCombat extends CombatScript {
    ...
    public boolean init(NPC npc) {
        npc.deathEndListener = ((entity, killer) -> {
            minions.forEach(MinionNpc::finish);
            objects.clear();
        });
    ...
    public void minionspawn(NPC npc, Entity target) {
        npc.addEvent(event -> {
            final Position pos = new Position(npc.getPosition().getX() + Misc.random(2), npc.getPosition().getY(), 0);
