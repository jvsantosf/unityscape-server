...
public final class BrimhavenAgility extends Controller {
	...
	private static class PlayingGame extends TimerTask {
		...
		private static Position taggedDispenser;


		private static Position getNextDispenser() {
			while(true) {
				Position tile = new Position(2761 + 11 * Utils.random(5) , 9546 + 11 * Utils.random(5) , 3);
		...
		private static void addIcon(Player player) {
			...
			if(taggedDispenser == null)
				return;
			player.getHintIconsManager().addHintIcon(taggedDispenser.getX(), taggedDispenser.getY(), taggedDispenser.getZ(), 65, 2, 0, -1, false);
		}
	...
	public boolean processObjectClick1(final WorldObject object) {
		if(object.getId() == 3581 || object.getId() == 3608) {
			...
			player.lock();
			player.setNextFacePosition(new Position(player.getX(), player.getY() -1, 3));
			player.animate(new Animation(1121));
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					...
					player.animate(new Animation(1122));
					final Position tile = new Position(player.getX(), player.getY() - rotationY, player.getZ());
					player.setNextForceMovement(new ForceMovement(tile, 1, 1));
					WorldTasksManager.schedule(new WorldTask() {
						@Override
						public void run() {
							player.setNextPosition(new Position(tile));
							this.stop();
							return;
						}
					}, 0, 1);
			...
			final Position tile = new Position(player.getX(), player.getY() - object.getRotation(), player.getZ());
			player.setNextForceMovement(new ForceMovement(tile, 1, 1));
			return false;
