...
public class SlidingTilesRoom extends PuzzleRoom {
	...
	public void openRoom() {
		...
		for (int i = 0; i < 9; i++) {
			if (shuffledNpcOrder[i] != 0) {
				...
				tiles[i] = new SlidingTile(shuffledNpcOrder[i], new Position(base.getX() + coords[0], base.getY() + coords[1], 0), manager, 0.0);
			}
		}
	}
	...
	public static class SlidingTile extends DungeonNPC {
		...
		public SlidingTile(int id, Position tile, DungeonManager manager, double multiplier) {
		...
		for (int i = 0; i < puzzle.tiles.length; i++) {
			if (puzzle.tiles[i] == npc) {
				...
				if (i == puzzle.solveOrder[puzzle.solveIndex - 1]) {
					...
					Position base = puzzle.manager.getRoomBaseTile(puzzle.reference);
					...
					player.applyHit(new Hit(player, (int) (player.getMaxHitpoints() * .2), Hit.HitLook.REGULAR_DAMAGE));
					return true;
				}
			}
		}
