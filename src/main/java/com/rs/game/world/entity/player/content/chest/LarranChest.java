...
public class LarranChest {
    ...
    private static final int KEY_ID = 53490; //brimstone key
    ...
    private static LootTable table = new LootTable().addItems(
            ...
            //rare
            //very rare
            new WeightedItem(28656, 1, 1),
            new WeightedItem(28649, 1, 1)
                ...
                if (item.getWeight() == 1) {
                    World.sendWorldMessage(
                            "<img=23><col=F39407>" + player.getDisplayName() + "<col=F39407> Received<col=F39407> Very rare "
                                    + item.getAmount() + "x " + item.getName() + "<col=F39407> from the larrans chest amount opened: " + player.larranschest,
                            false);
                }
