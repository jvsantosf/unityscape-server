...
import com.rs.game.world.entity.player.content.Slayer.SlayerTask;
...
public class SlayerManager implements Serializable {
	...
	public int completedTasks, slayerPoints, maximumTaskCount, currentTaskCount;
	...
	public boolean hasMaster(SlayerMaster master) {
		if (currentMaster == null)
			return false;
		return master == currentMaster;
	}
	
	public boolean hasTask(SlayerTask task) {
		if (currentTask == null)
			return false;
	...
	private void cancleCurrentTask() {
		if (currentTask == null) {
			...
			int cost = player.getDonationManager().hasRank(DonatorRanks.DRAGON) ? 15 : 30;
			player.getDialogueManager().startDialogue(new Dialogue() {

				@Override
				public void start() {
					sendOptionsDialogue("What would you like to pay with?", "Slayer points (" + cost + ")", "Coins (" + (cost * 10) +"k)", "Nevermind.");
				}
				...
				public void run(int interfaceId, int componentId) {
					if (componentId == OPTION_1) {
						if (removePoints(cost)) {
							...
							player.getPackets().sendGameMessage("You don't have enough slayer points to do that.");
						}
						end();
					} else if (componentId == OPTION_2) {
						if (player.getInventory().removeItemMoneyPouch(new Item(995, cost * 10000))) {
							skipCurrentTask();
							setCurrentTask(true);
							player.getPackets().sendGameMessage("Your slayer task has been re-assigned as requested, as a result, your slayer-streak has been reset to 0.");
						} else {
							player.getPackets().sendGameMessage("You can't afford to do that.");
						}
						end();
	...
	public void checkCompletedTask(int damageAdmitted, int otherDamageAdmitted) {
		...
		player.getSkills().addXp(Skills.SLAYER, damageAdmitted / 3);
		player.getPackets().sendRunScript(-6, getCount());
		if (currentTaskCount + otherSocialCount == maximumTaskCount) {
			...
			.sendGameMessage("You have finished your slayer task, talk to a slayer master for a new one.");
			resetTask(true, true);
			if (player.getSkills().getLevel(Skills.SLAYER) < 65) {
				//player.setSlayerPoints(player.getSlayerPoints() + 5);
				setPoints(slayerPoints + 3);
			}
			player.kuradalTasks++;
			if (player.getSkills().getLevel(Skills.SLAYER) > 65 && player.getSkills().getLevel(Skills.SLAYER) != 99) {
				setPoints(slayerPoints + 6);
			}
			player.kuradalTasks++;

			if (player.getSkills().getLevel(Skills.SLAYER) == 99) {
				setPoints(slayerPoints + 9);
			}
			player.kuradalTasks++;
			if (Player.isThursday() && !player.finishedTask) {
				player.setTasksFinished(player.getTasksFinished() + 1);
				if (player.getTasksFinished() >= 4 && !player.sendTaskMSG) {
					player.sm("<col=8B0000>You have finished 5 Slayer tasks for todays daily challange!");
					player.sendTaskMSG = true;
				} else if (player.getTasksFinished() >= 4 && player.getCorpsKilled() >= 15 && player.getAddedWell() >= 5 && !player.finishedTask) {
					...
					player.sm("<col=8B0000>You have finished the daily task & claimed your reward!");
	...
	public void invitePlayer(Player otherPlayer) {
		if (currentTask != null) {
			...
			return;
		} else if (otherPlayer == null || !otherPlayer.withinDistance(player, 7) || player.isFinished()
				|| otherPlayer.isFinished()) {
	...
	public void setCurrentTask(SlayerTask task, int maximumTaskCount) {
		...
		if (currentTask != null)
			player.getPackets().sendRunScript(-5, maximumTaskCount, currentTaskCount, currentTask.getName());
		else
			player.getPackets().sendRunScript(-6, 0);
	}
