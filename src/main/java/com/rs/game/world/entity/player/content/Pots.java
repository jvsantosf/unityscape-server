...
import com.rs.game.world.entity.player.controller.impl.CrucibleControler;
...
public final class Pots {
	...
	public static enum Pot {
		...
		AGGRESSION_POTION(new int[] { 28660, 28662, 28664, 28666 }, Effects.AGGRESSION_POTION),

		;

		private int[] id;
		private Effects effect;
		public static final Pot[] VALUES = Pot.values();
	...
	private enum Effects {
		...
		WEAK_STAT_CURE_POTION() {
			...
			public void extra(Player player) {
				...
				player.getToxin().applyImmunity(ToxinType.POISON, 500);
			}
		},
		...
		STRONG_STAT_CURE_POTION() {
			...
			public void extra(Player player) {
				...
				player.getToxin().applyImmunity(ToxinType.POISON, 2000);
			}
		},
		...
		SANFEW_SERUM(Skills.ATTACK, Skills.STRENGTH, Skills.DEFENCE, Skills.MAGIC, Skills.RANGE, Skills.AGILITY, Skills.COOKING, Skills.CRAFTING, Skills.FARMING, Skills.FIREMAKING, Skills.FISHING, Skills.FLETCHING, Skills.HERBLORE, Skills.MINING, Skills.RUNECRAFTING, Skills.SLAYER, Skills.SMITHING, Skills.THIEVING, Skills.WOODCUTTING, Skills.SUMMONING) {
			...
			public void extra(Player player) {
				...
				player.getToxin().applyImmunity(ToxinType.POISON, player.getDonationManager().hasRank(DonatorRanks.RUNE) ? 600 : 300);
			}
		...
		ANTIPOISON_PLUS() {
			@Override
			public void extra(Player player) {
				player.getToxin().applyImmunity(ToxinType.POISON, player.getDonationManager().hasRank(DonatorRanks.RUNE) ? 1800 : 900);
			}
		},
		ANTIPOISON_DOUBLE_PLUS() {
			@Override
			public void extra(Player player) {
				player.getToxin().applyImmunity(ToxinType.POISON, player.getDonationManager().hasRank(DonatorRanks.RUNE) ? 2400 : 1200);
			}
		},
		SUPER_ANTIPOISON() {
			@Override
			public void extra(Player player) {
				player.getToxin().applyImmunity(ToxinType.POISON, player.getDonationManager().hasRank(DonatorRanks.RUNE) ? 1200 : 600);
				player.getToxin().applyImmunity(ToxinType.VENOM, 600);
			}
		},
		ENERGY_POTION() {
			@Override
		...
		ANTI_FIRE() {
			...
			public void extra(final Player player) {
				player.getBuffManager().applyBuff(new Buff(BuffType.ANTI_FIRE, player.getDonationManager().hasRank(DonatorRanks.RUNE) ? 1200 : 600, true));
			}
		},
		SUPER_ANTI_FIRE() {
			@Override
			public void extra(final Player player) {
                player.getBuffManager().applyBuff(new Buff(BuffType.SUPER_ANTI_FIRE, player.getDonationManager().hasRank(DonatorRanks.RUNE) ? 1200 : 600, true));
			}
		},
		AGGRESSION_POTION() {
			@Override
			public void extra(final Player player) {
				player.getBuffManager().applyBuff(new Buff(BuffType.AGGRESSION_POTION, 600, true));
			}
		},
		...
		SUPER_GUTHIX_REST() {
			...
			@Override
			public void extra(Player player) {
				player.getToxin().applyImmunity(ToxinType.POISON, 2);
		...
		EXTREME_MAG_POTION(Skills.MAGIC) {
			...
			public int getAffectedSkill(Player player, int skillId, int actualLevel, int realLevel) {
				int level = actualLevel > realLevel ? realLevel : actualLevel;
				return (int) (level + 3 + (realLevel * 0.9));
			}
		},
		RECOVER_SPECIAL() {
		...
		SARADOMIN_BREW("You drink some of the foul liquid.", Skills.ATTACK, Skills.DEFENCE, Skills.STRENGTH, Skills.MAGIC, Skills.RANGE) {
			...
			public void extra(Player player) {
				...
				player.heal(10, hitpointsModification);
			}
		},
		...
		SUPER_SARADOMIN_BREW("You drink some of the foul liquid.", Skills.ATTACK, Skills.DEFENCE, Skills.STRENGTH, Skills.MAGIC, Skills.RANGE) {
			...
			public void extra(Player player) {
				int hitpointsModification = (int) (player.getMaxHitpoints() * 0.10);
				player.heal(13, hitpointsModification);
			}
		},
		...
		OVERLOAD() {
			...
			public boolean canDrink(Player player) {
				...
				if (player.getHitpoints() <= 50 || player.getOverloadDelay() > 480) {
					player.getPackets()
					.sendGameMessage(
							"You need more than 50 life points to survive the power of an overload potion.");
					return false;
				}
				return true;
			}
			...
			@Override
			public void extra(final Player player) {
				player.setOverloadDelay(player.getDonationManager().hasRank(DonatorRanks.RUNE) ? 1200 : 600);
				player.getPackets().sendRunScript(-7, 0, 15332, player.getOverloadDelay() * 600);
		...
		PRAYER_RENEWAL() {
			@Override
			public void extra(Player player) {
				player.getBuffManager().applyBuff(new Buff(BuffType.PRAYER_RENEWAL, player.getDonationManager().hasRank(DonatorRanks.RUNE) ? 1200 : 600, true));
			}
		},
