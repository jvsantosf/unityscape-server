...
public class SangStaff implements Serializable {
	...
	public static final int CHARGED_SANG = 29784;
	private static final int UNCHARGED_SANG = 29790;
	...
	public void charge(Item used, Item usedWith) {
		...
		blood_rune += added;
		if (usedWith.getId() == UNCHARGED_SANG) {
			usedWith.setId(CHARGED_SANG);
	...
	public boolean degrade() {
		if (blood_rune <= 0) {
			player.getEquipment().getItems().get(Equipment.SLOT_WEAPON).setId(UNCHARGED_SANG);
	 ...
	 * @return
	 */
	public static boolean isSangStaff(int itemId) {
		return (itemId == CHARGED_SANG);
	}
