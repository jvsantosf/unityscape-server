...
public final class Dungeon {
	...
	//	Settings.DEBUG = true;
		long lastDung = com.rs.utility.Utils.currentTimeMillis();
		...
		while (true) {
			System.out.println("TOOK: " + (com.rs.utility.Utils.currentTimeMillis() - lastDung) + ", ");
			lastDung = com.rs.utility.Utils.currentTimeMillis();
...
//					g.drawString("\033" + tier, x * 20 + 36, (7 - y) * 20 + 49);
	...
	public Dungeon(DungeonManager manager, int floorId, int complexity, int size) {
		...
		for (RoomNode node : children) {
			...
			if (node.key != -1)
				map[node.x][node.y].setDropId(DungeonConstants.KeyDoors.values()[node.key].getKeyId());
			for (int doorDir = 0; doorDir < map[node.x][node.y].getRoom().getDoorDirections().length; doorDir++) {
				...
				RoomNode neighbor = structure.getRoom(node.x + com.rs.utility.Utils.ROTATION_DIR_X[rotation], node.y + com.rs.utility.Utils.ROTATION_DIR_Y[rotation]);
				if (neighbor.parent == node) {
					if (puzzle) {
						...
						DungeonConstants.SkillDoors sd = DungeonConstants.SkillDoors.values()[doorIndex];
