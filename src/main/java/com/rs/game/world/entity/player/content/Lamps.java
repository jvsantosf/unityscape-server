...
public class Lamps {
			...
			public void run(int interfaceId, int componentId) {
				if (componentId >= 13 && componentId <= 37) {
					...
					if (skill != Skills.CONSTRUCTION) {
						...
						selectedSkill = skill;
						sendSelectedSkill();
						player.getPackets().sendUnlockIComponentOptionSlots(1263, 43, -1, 0, true, 0); // not the right
																										// way but eh
					}
					...
					double exp = player.getSkills().addXpLamp(selectedSkill,
							getExp(player.getSkills().getLevelForXp(selectedSkill), selectableLampType(id)));
					player.getInterfaceManager().removeScreenInterface();
					sendDialogue("<col=0000ff>Your wish has been granted!</col>", "You have been awarded "
							+ Utils.getFormattedNumber(exp, ',') + " XP in " + Skills.SKILL_NAME[selectedSkill] + "!");
	...
	private static void openSkillDialog(Player player, final int slot, final int id) {
		...
		player.getDialogueManager().startDialogue(new Dialogue() {
			...
			public void run(int interfaceId, int componentId) {
				...
				player.getInventory().deleteItem(slot, new Item(id, 1));
				double exp = player.getSkills().addXpLamp(skillId,
	...
	private static double getExp(int skillLevel, int lampType) {
		...
		if (skillLevel <= BASE_LAMPS_XP.length)
			xp = BASE_LAMPS_XP[skillLevel - 1] * 12;
		else
			xp = BASE_LAMPS_XP[BASE_LAMPS_XP.length - 1] * 12;
