...
public class ToxinMaze extends PuzzleRoom {
	...
	public boolean processObjectClick1(final Player player, WorldObject object) {
		...
		if (object.getId() == BARRIER_ENTRANCE && toxinTask == null && !isComplete()) {
			...
			Position in = new Position(object.getX() + Utils.ROTATION_DIR_X[object.getRotation()], object.getY() + Utils.ROTATION_DIR_Y[object.getRotation()], 0);
			Position out = new Position(object.getX(), object.getY(), 0);
			...
			final Position target_ = target;
			player.lock(delay + 1);
			WorldTasksManager.schedule(new WorldTask() {
				@Override
				public void run() {
					Position fromTile = new Position(player.getX(), player.getY(), player.getZ());
					player.setNextPosition(target_);
					player.setNextForceMovement(new ForceMovement(fromTile, 0, target_, 1, getFaceDirection(target_, player)));
			...
			if (ticks > 0) {
				...
				for (Player player : manager.getParty().getTeam()) {
					if (manager.getCurrentRoomReference(player).equals(reference)) {
						player.applyHit(new Hit(player, (int) (player.getMaxHitpoints() * .1), Hit.HitLook.REGULAR_DAMAGE));
