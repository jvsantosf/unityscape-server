...
import com.rs.game.map.OwnedObjectManager.ProcessEvent;
...
public class DwarfMultiCannon {
	...
	public static void setUp(Player player) {
		if (OwnedObjectManager.containsObjectValue(player, CANNON_OBJECTS)) {
			player.getPackets().sendGameMessage("You can only have one cannon setup at the same time.");
			return;
		}
		...
		Position pos = player.transform(-3, -3, 0);
		if (!World.isTileFree(pos.getZ(), pos.getX(), pos.getY(), 3)) {
		...
		player.lock();
		player.setNextFacePosition(pos);
		OwnedObjectManager.addOwnedObjectManager(player, objects, cycles, new ProcessEvent() {
			...
			public void process(Player player, WorldObject currentObject) {
				if (step != CANNON_PIECES.length || !player.clientHasLoadedMapRegion() || player.isFinished())
					return;
				...
				if (player.getCannonBalls() == 0)
					return;
				rotation++;
				if (rotation == CANNON_EMOTES.length)
					rotation = 0;
//				if (rotation % 2 == 0)
//					return;
				World.sendObjectAnimation(player, currentObject, new Animation(CANNON_EMOTES[rotation]));
				NPC nearestN = null;
				double lastD = Integer.MAX_VALUE;
				int angle = getAngle(rotation);
				...
				for (int regionId : player.getMapRegionsIds()) {
					...
					for (int npcIndex : npcIndexes) {
						...
						if (npc == null || npc == player.getFamiliar() || npc.isDead() || npc.isFinished() || npc.getZ() != currentObject.getZ() || !Utils.isOnRange(npc.getX(), npc.getY(), npc.getSize(), currentObject.getX(), currentObject.getY(), objectSizeX, 10) || !npc.getDefinitions().hasAttackOption() || !npc.clipedProjectile(currentObject, false) || npc.isCantInteract())
							continue;
				...
				if (nearestN != null) {
					double hitChance = 90;
					int damage = hitChance < Utils.random(100) ? 0 : Utils.random(350);
					...
					if (twoBalls) {
						damage = hitChance < Utils.random(100) ? 0 : Utils.random(350);
	...
	private static WorldObject getObject(int i, Position tile) {
		return new WorldObject(CANNON_OBJECTS[i], 10, 0, tile.getX(), tile.getY(), tile.getZ());
	}
