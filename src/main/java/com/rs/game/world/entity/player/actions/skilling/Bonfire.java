...
public class Bonfire extends Action {
	...
	public boolean process(Player player) {
		if(checkAll(player)) {
			if(Utils.random(500) == 0) {
				new FireSpirit(new Position(object, 1), player);
	...
	public int processWithDelay(Player player) {
		if (player.getEquipment().wearingSkillCape(Skills.FIREMAKING) && Utils.getRandom(100) <= 50) {
			player.sendMessage("Your firemaking cape saves you a log.");
		} else {
		...
		if(count++ == 4 && player.getLastBonfire() == 0) {
			player.getBuffManager().applyBuff(new Buff(BuffType.BONFIRE, log.boostTime * 100, false));
