...
public class TrollInvasion extends Controller {
	...
	private final Position OUTSIDE_LOCATION = new Position(2877, 3561, 0);
	...
	public void setEohric(int type) {
		...
		if (eohric == null && type == 2)
			return;
		if (type == 1) {
			World.spawnNPC(13700, new Position(getWorldTile(43, 29)), -1,
					0, true, true);

		} else if (type == 2) {
			eohric.faceObject(new WorldObject(9270, 10, 0, new Position(
					getWorldTile(42, 29))));
	...
	public Position getWorldTile(int mapX, int mapY) {
		return new Position(boundChuncks[0] * 8 + mapX, boundChuncks[1] * 8
				+ mapY, 0);
	}
					...
					if (tableDestroyed == true) {
						...
						if (World.getNPC(EOHRIC) != null) {
							World.getNPC(EOHRIC).faceObject(
									new WorldObject(9270, 10, 0, new Position(
											getWorldTile(42, 29))));
							World.getNPC(EOHRIC).animate(
									new Animation(1559));
						}
					}
				} else if (loop == 3) {
	...
	public void leaveGame(int type) {
		...
		Position outside = new Position(OUTSIDE_LOCATION);
		if (type == 0 || type == 2)
	...
	public void commenceGame() {
		...
		FadingScreen.unfade(player, time, new Runnable() {
			@Override
			public void run() {
				player.setNextPosition(new Position(getWorldTile(39, 21)));
	...
	public void loadSettings() {
		...
		World.spawnObject(new WorldObject(9242, 10, 0, getWorldTile(42, 32)),
				true);
		// npcs
		World.spawnNPC(13699, new Position(getWorldTile(42, 19)), -1, 0,
				true, true);
		World.spawnNPC(13701, new Position(getWorldTile(44, 25)), -1, 0,
				true, true);
	}
	...
	public void processDynamite() {
		...
		if (tableDestroyed == false) {
			...
			WorldObject table = new WorldObject(9270, 10, 0, new Position(
					getWorldTile(42, 29)));
		...
		if (complexity) {
			for (int enemy : ADVANCED_WAVES[wave - 1]) {
				...
				if (closeEnemies.withinDistance(object, 6) && hpPercent < 5
					...
					archer.animate(new Animation(12658));
