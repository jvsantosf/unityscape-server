...
import com.rs.game.world.entity.npc.dungeonnering.YkLagorMage;
import com.rs.game.world.entity.npc.dungeonnering.YkLagorThunderous;
...
public class YkLagorThunderousCombat extends CombatScript {
			...
			if (boss.getNextAttack() == 0) {
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						cycles++;
						if (cycles == 3) {
							boss.animate(new Animation(19588));
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						...
						if (cycles == 2) {
							boss.animate(new Animation(19587));
	...
	public static void sendPullAttack(final Position tile, final Player player, final boolean disablePrayers) {
		player.lock(3);
	...
	public static void sendMagicalAttack(YkLagorThunderous npc, boolean specialAttack) {
		npc.animate(new Animation(19590));
