...
import com.rs.game.world.entity.npc.fightpits.FightPitsNPC;
import com.rs.game.world.entity.npc.fightpits.TzKekPits;
...
public final class FightPits {
	...
	private static Position[] GAME_TELEPORTS = {
			new Position(4577, 5086, 0),
			new Position(4571, 5083, 0),
			new Position(4564, 5086, 0),
			new Position(4564, 5097, 0),
			new Position(4571, 5101, 0),
			new Position(4578, 5097, 0)};
				...
				synchronized (lock) {
					if(!startedGame) {
						...
						if(minutes == 0) {
							...
							for(int i = 0; i < 10; i++)
								spawns.add(new FightPitsNPC(2734, new Position(GAME_TELEPORTS[Utils.random(GAME_TELEPORTS.length)], 3)));
						}else if (minutes == 6) { //spawn tz-kek
							for(int i = 0; i < 10; i++)
								spawns.add(new TzKekPits(2736, new Position(GAME_TELEPORTS[Utils.random(GAME_TELEPORTS.length)], 3)));
						}else if (minutes == 7) { //spawn tok-xil
							for(int i = 0; i < 10; i++)
								spawns.add(new FightPitsNPC(2739, new Position(GAME_TELEPORTS[Utils.random(GAME_TELEPORTS.length)], 3)));
	...
	public static void enterArena(Player player) {
		...
		player.setNextPosition(new Position(GAME_TELEPORTS[Utils.random(GAME_TELEPORTS.length)], 3));
			...
			if(type == 0) {
				...
				if(player.hasSkull() && player.getSkullId() == 1) {//if has champion skull
					...
					if(!player.getInventory().addItem(6529, tokkul) && type == 1) 
						World.addGroundItem(new Item(6529, tokkul), new Position(4585, 5076, 0), player, true, 180, true);
				}
				if(type == 1) {
					player.lock(5);
					player.addWalkSteps(4585, 5076, 5, false);
				}else if(type == 2)
					player.setNextPosition(new Position(new Position(4592, 5073, 0), 2));
			}
			refreshFoes();
			checkPlayersAmmount();
