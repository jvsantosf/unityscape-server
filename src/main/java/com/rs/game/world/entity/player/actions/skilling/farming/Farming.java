...
public class Farming implements Serializable {
			...
			if (Trees.trees.containsKey(sapling)) {
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						if (stage == 0) {
							player.lock();
							player.animate(PLANT_ANIMATION);
							if (objectId == Patches.LUMMY_TREE_PATCH.getObjectId()) {
	...
	public static void harvestTreeSapling(final Player player, final int objectId, int sapling) {
		...
		if (player.getInventory().getFreeSlots() < 3) {
			...
			case 13661:
				player.animate(INFERNO_ADZE_CHOP);
				break;
				
			case 6739:
				player.animate(DRAGON_AXE_CHOP);
				break;
				
			case 1359:
				player.animate(RUNE_AXE_CHOP);
				break;
				
			case 1357:
				player.animate(ADAMANT_AXE_CHOP);
				break;
				
			case 1355:
				player.animate(MITHRIL_AXE_CHOP);
				break;
				
			case 1361:
				player.animate(BLACK_AXE_CHOP);
				break;
				
			case 1353:
				player.animate(STEEL_AXE_CHOP);
				break;
				
			case 1349:
				player.animate(IRON_AXE_CHOP);
				break;
				
			case 1351:
				player.animate(BRONZE_AXE_CHOP);
				break;
			}
			int amount = 3 + (player.getDonationManager().hasRank(DonatorRanks.ADAMANT) ? 2 : 0);
	...
	public static void harvestHerb(final Player player, final int objectId, int seed) {
		...
		if (player.getInventory().getFreeSlots() < 3) {
			player.getPackets().sendGameMessage("You need atleast three free inventory spaces to harvest.");
		} else if (player.getInventory().containsItemToolBelt(SECATEURS)) {
			int amount = (player.getEquipment().wearingSkillCape(Skills.FARMING) ? 6 : 3) + (player.getDonationManager().hasRank(DonatorRanks.ADAMANT) ? 2 : 0) ;
			double xp = amount * herb.getHarvestXp();
			if (amount < 3 && player.getInventory().getFreeSlots() <= 3) {
				player.animate(PICK_HERB);
				player.getSkills().addXp(Skills.FARMING, xp * 3);
				if (player.getInventory().containsItem(18336, 1)) {
					int chance = Misc.random(10);
					if (chance == 1) {
						player.getInventory().addItemDrop(herb.getSeed(), 1);
					}
				}
				player.getInventory().addItemDrop(herb.getProduct(), 3);
				growWeeds(player, objectId);
				if (objectId == Patches.FALADOR_HERB.getObjectId()) {
				...
				player.getInventory().addItemDrop(herb.getProduct(), amount);
				growWeeds(player, objectId);
				if (objectId == Patches.FALADOR_HERB.getObjectId()) {
					player.setFaladorHerbPatch(-1);
					player.setFaladorHerbPatchRaked(false);
