...
public class NomadCombat extends CombatScript {
	...
	private void spawnFlameVortex(Position tile) {
		...
		if(nomad.isMeleeMode()) {
			...
			if(target instanceof Player && nomad.useSpecialSpecialMove()) {
				...
				case 0:
					...
					final Position middle = new Position(player);
					WorldTasksManager.schedule(new WorldTask() {
						int count;
						@Override
						public void run() {
				...
				case 1:
					...
					Position throne = nomad.getThroneTile();
					...
					WorldTasksManager.schedule(new WorldTask() {
						...
						public void run() {
							if(!secondLoop) {
								...
								player.setNextFacePosition(new Position(player.getX(), player.getY()+1, 0));
