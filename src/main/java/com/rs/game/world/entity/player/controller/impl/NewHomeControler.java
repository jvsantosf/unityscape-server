...
public class NewHomeControler extends Controller {
    ...
    private void sendGoodActions() {
        ...
        NPC2.animate(new Animation(0x35F));// waving bye
        WorldTasksManager.schedule(new WorldTask() {

            @Override
            public void run() {// teleporting away
                NPC2.animate(new Animation(2111));
    ...
    private void startNPCActions() {
        player.lock();
        NPC1 = new NPC(1, new Position(2966, 3386, 0), -1, 0, true, true);
        NPC2 = new NPC(1, new Position(2966, 3387, 0), -1, 0, true, true);
        NPC1.faceEntity(NPC2);
        NPC1.animate(new Animation(5078));
