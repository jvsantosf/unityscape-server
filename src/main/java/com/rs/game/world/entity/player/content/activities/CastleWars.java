...
import com.rs.game.world.entity.player.content.dialogue.impl.CastleWarsScoreboard;
import com.rs.game.world.entity.player.controller.impl.CastleWarsPlaying;
...
public final class CastleWars {
    ...
    public static final Position LOBBY = new Position(2442, 3090, 0),
	    SARA_WAITING = new Position(2381, 9489, 0),
	    ZAMO_WAITING = new Position(2421, 9523, 0),
	    SARA_BASE = new Position(2426, 3076, 1),
	    ZAMO_BASE = new Position(2373, 3131, 1);
...
//	}
	...
	player.setNextPosition(new Position(team == ZAMORAK ? ZAMO_WAITING : SARA_WAITING, 1));
	...
	setCape(player, null);
	//setHood(player, null);
	player.setNextPosition(new Position(LOBBY, 2));
	...
	if (weaponId == 4037 || weaponId == 4039) {
	    ...
	    CastleWars.dropFlag(player.getLastPosition(), weaponId == 4037 ? CastleWars.SARADOMIN : CastleWars.ZAMORAK);
	}
	...
	player.useStairs(-1, new Position(LOBBY, 2), 0, 1);
	if (playingGame != null && waiting[team].size() == 0 && playing[team].size() == 0)
	    destroyPlayingGame(); // cancels if 0 players playing/waiting on any
	// of the tea
    }
	...
	player.setNextPosition(new Position(team == ZAMORAK ? ZAMO_BASE : SARA_BASE, 1));
    }
    ...
    public static void dropFlag(Position tile, int flagTeam) {
	if (playingGame == null)
	    return;
    ...
    public static boolean isBarricadeAt(Position tile) {
	if (playingGame == null)
	    return false;
	...
	public boolean isBarricadeAt(Position tile) {
	...
	public void addBarricade(int team, Player player) {
	    ...
	    barricades.add(new CastleWarBarricade(team, new Position(player)));
	}
	...
	public void takeFlag(Player player, int team, int flagTeam, WorldObject object, boolean droped) {
	    ...
	    if (!droped) {
		WorldObject flagStand = new WorldObject(flagTeam == SARADOMIN ? 4377 : 4378, object.getType(), object.getRotation(), object.getX(), object.getY(), object.getZ());
	...
	public void dropFlag(Position tile, int flagTeam) {
	    removeHintIcon(flagTeam);
	    WorldObject flagDroped = new WorldObject(flagTeam == SARADOMIN ? 4900 : 4901, 10, 0, tile.getX(), tile.getY(), tile.getZ());
