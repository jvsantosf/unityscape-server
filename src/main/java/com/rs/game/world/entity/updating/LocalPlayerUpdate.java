package com.rs.game.world.entity.updating;
...
public final class LocalPlayerUpdate {
	...
	private boolean needsRemove(Player p) {
		return (p.isFinished() || !player.withinDistance(p, player.hasLargeSceneView() ? 126 : 14));
	}

	private boolean needsAdd(Player p) {
		return p != null && !p.isFinished() && player.withinDistance(p, player.hasLargeSceneView() ? 126 : 14) && localAddedPlayers < MAX_PLAYER_ADD;
	}
	...
	private void processLocalPlayers(OutputStream stream,
		...
		for (int i = 0; i < localPlayersIndexesCount; i++) {
			...
			if (needsRemove(p)) {
				...
				regionHashes[playerIndex] = p.getLastPosition() == null ? p.getRegionHash() : p.getLastPosition().getRegionHash();
				int hash = p.getRegionHash();
				if (hash == regionHashes[playerIndex])
					stream.writeBits(1, 0);
				else {
				...
				if (p.hasTeleported()) {
					...
					stream.writeBits(2, 3);
					int xOffset = p.getX() - p.getLastPosition().getX();
					int yOffset = p.getY() - p.getLastPosition().getY();
					int planeOffset = p.getZ()
							- p.getLastPosition().getZ();
					if (Math.abs(p.getX() - p.getLastPosition().getX()) <= 14 //14 for safe
							&& Math.abs(p.getY() - p.getLastPosition().getY()) <= 14) { //14 for safe
						stream.writeBits(1, 0);
