...
import com.rs.game.world.entity.npc.others.YellowWizard;
...
public class RunespanControler extends Controller {
	public static Position WIZARD_TOWER = new Position(3106, 6160, 1);
	public static Position LOWER_LEVEL = new Position(3994, 6105, 1);
	public static Position HIGHER_LEVEL = new Position(4149, 6104, 1);
	public static Position VINE_LADDER = new Position(3957, 6106, 1);
	private static final Position[] RANDOM_LOCATIONS = {
			new Position(3959, 6104, 1),
			new Position(3996, 6055, 1),
			new Position(3984, 6086, 1),
			new Position(4399, 6041, 1)
	...
	private static enum HandledPlatforms {
		...
		private Position smallIsland, largeIsland;
		
		private HandledPlatforms(int largeIslandX, int largeIslandY, int smallIslandX, int smallIslandY) {
			largeIsland = new Position(largeIslandX, largeIslandY, 1);
			smallIsland = new Position(smallIslandX, smallIslandY, 1);
		}
		
		private static Object[] getToPlataform(Position fromPlataform) {
	...
	private boolean handleCrossingPlataform(final WorldObject object, final Platforms plataform) {
		...
		final Position toTile = (Position) toPlataform[0];
		player.lock(7);
	...
	public void handleWizzard(Player player) {
		startTime = Utils.currentTimeMillis();
		yellowWizard = new YellowWizard(new Position(RANDOM_LOCATIONS[Utils.random(RANDOM_LOCATIONS.length)]), this);
	...
	public boolean processNPCClick3(NPC npc) {
		if(npc.getId() == 15433 && yellowWizard != null) {
			Position tile = yellowWizard;
			player.getHintIconsManager().addHintIcon(tile.getX(), tile.getY(), tile.getZ(), 150, 2, 0, -1, true);
			return false;
		}
		return true;
	}
	...
	public boolean processObjectClick1(final WorldObject object) {
		if(object.getId() == 70507) {
			if(object.getX() == 4367 && object.getY() == 6062) {
				...
				player.lock(35);
				final Position dest = new Position(4367, 6033, 1);
				WorldTasksManager.schedule(new WorldTask() {
					private int stage;
					@Override
					public void run() {
						if(stage == 0) {
							player.setNextFacePosition(new Position(4367, 6062, 1));
				...
				player.addWalkSteps(object.getX(), object.getY(), 0, false);
				player.lock(35);
				final Position dest = new Position(4367, 6062, 1);
				WorldTasksManager.schedule(new WorldTask() {
					private int stage;
					@Override
					public void run() {
