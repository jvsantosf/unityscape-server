...
import com.rs.game.mysql.Database;
...
public class Highscores implements Runnable {
	...
	public void run() {
		try {
			Database db = new Database("http://www.realityx718.com/", "realityx_hscore", "mohammed1+++", "realityx_hscore");
			...
			if (!db.init()) {
				...
				System.err.println("Could not add "+name+" to the highscores. Database could not connect.");
				return;
			}
