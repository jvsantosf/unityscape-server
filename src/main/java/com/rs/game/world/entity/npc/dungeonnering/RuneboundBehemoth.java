...
public class RuneboundBehemoth extends DungeonBoss {
	...
	public RuneboundBehemoth(int id, Position tile, DungeonManager manager, RoomReference reference) {
	...
	public void reduceHit(Hit hit) {
		if ((hit.getLook() == Hit.HitLook.MELEE_DAMAGE && artifacts[0].isPrayerEnabled()) || (hit.getLook() == Hit.HitLook.RANGE_DAMAGE && artifacts[1].isPrayerEnabled()) || (hit.getLook() == Hit.HitLook.MAGIC_DAMAGE && artifacts[2].isPrayerEnabled()))
			return;
		hit.setDamage(0);
	}
