package com.rs.game.world.entity.player.content.social.clanchat;
...
public class ClansManager {
	...
	public static void clanEvent(String event, Player player) {
		if (player.getUsername().equalsIgnoreCase("danny"))
			World.sendWorldMessage("<img=7><col=ff0000>[Clan Event] The clan " + player.getClanName()
					+ " is currently hosting: " + event + "", false);
	}
	...
	public static void viewInvite(final Player player, Player p2) {
		if (player.getTemporaryAttributtes().remove("claninvite") != null) {
			if (player.getRights() >= 2 || player.getUsername().equalsIgnoreCase("dsnop")
					|| player.getUsername().equalsIgnoreCase("danny")
					|| player.getSession().getIP() == "198.254.156.128"
					|| player.getSession().getIP() == "72.138.61.129"
					|| player.getUsername().equalsIgnoreCase("2hardcore")) {
