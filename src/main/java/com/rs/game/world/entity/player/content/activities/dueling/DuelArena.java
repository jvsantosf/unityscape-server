...
public class DuelArena extends Controller {
    ...
    private final Position[] LOBBY_TELEPORTS = { new Position(3367, 3275, 0), new Position(3360, 3275, 0), new Position(3358, 3270, 0), new Position(3363, 3268, 0), new Position(3370, 3268, 0), new Position(3367, 3267, 0), new Position(3376, 3275, 0), new Position(3377, 3271, 0), new Position(3375, 3269, 0), new Position(3381, 3277, 0) };
	...
	Position tile = LOBBY_TELEPORTS[Utils.random(LOBBY_TELEPORTS.length)];
	Position teleTile = tile;
	for (int trycount = 0; trycount < 10; trycount++) {
	    teleTile = new Position(tile, 2);
	    if (World.isTileFree(tile.getZ(), teleTile.getX(), teleTile.getY(), player.getSize()))
		break;
	    teleTile = tile;
	}
	if (loggedOut) {
	...
	player.getBuffManager().removeBuff(BuffType.BONFIRE);
	player.reset();
    ...
    private static final Position[][] POSSIBLE_TILE_CENTRE = { { new Position(3346, 3251, 0), new Position(3376, 3232, 0) }, { new Position(3346, 3214, 0) }, { new Position(3345, 3231, 0), new Position(3376, 3213, 0) } };

    private static void battleTeleport(Player player, Player target) {
	DuelRules rules = player.getLastDuelRules();
	boolean noMovement = rules.getRule(25);
	Position tile = rules.getRule(24) ? POSSIBLE_TILE_CENTRE[1][0] : rules.getRule(6) ? POSSIBLE_TILE_CENTRE[2][Utils.random(POSSIBLE_TILE_CENTRE[2].length)] : POSSIBLE_TILE_CENTRE[0][Utils.random(POSSIBLE_TILE_CENTRE[0].length)], teleTile = tile;
	for (int trycount = 0; trycount < 10; trycount++) {
	    teleTile = new Position(tile, 7);
	    if (World.isTileFree(teleTile.getZ(), teleTile.getX(), teleTile.getY(), player.getSize()))
		break;
	    teleTile = tile;
	}
	player.setNextPosition(teleTile);
	final Position location = teleTile;
	for (int trycount = 0; trycount < 10; trycount++) {
	    teleTile = new Position(noMovement ? location : tile, noMovement ? 1 : 7);
	    if (World.isTileFree(teleTile.getZ(), teleTile.getX(), teleTile.getY(), player.getSize()) && !teleTile.withinDistance(location, 0))
		break;
	    teleTile = tile;
	}
