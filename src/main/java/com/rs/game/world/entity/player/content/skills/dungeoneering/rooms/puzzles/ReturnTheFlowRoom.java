...
public class ReturnTheFlowRoom extends PuzzleRoom {
	...
	public void openRoom() {
		...
		for (int i = 0; i < 4; i++) {
			...
			FlowBuilder.FlowPiece node = flows[i].flow.start;
		...
		if (object.getId() == RUBBLE_PIECE[type] || object.getId() == RUBBLE_PIECE_FLOW[type]) {
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					...
					FlowBuilder.FlowPiece node = findFlowPiece(coords[0], coords[1]);
	...
	private void startFlow(FlowBuilder.FlowPiece node) {
		while (node.next != null) {
			if (node.corner) {
	...
	private FlowBuilder.FlowPiece findFlowPiece(int x, int y) {
		for (Flow flow : flows) {
			FlowBuilder.FlowPiece node = flow.flow.start;
			while (node.next != null) {
