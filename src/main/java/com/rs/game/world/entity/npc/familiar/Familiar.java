...
public abstract class Familiar extends NPC implements Serializable {
	...
	public Familiar(Player owner, Pouches pouch, Position tile,
			int mapAreaNameHash, boolean canBeAttackFromOutOfArea) {
		super(pouch.getNpcId(), tile, mapAreaNameHash,
				0, canBeAttackFromOutOfArea, false);
		this.owner = owner;
		this.pouch = pouch;
		resetTickets();
		specialEnergy = 60;
	...
	public void resetTickets() {
		ticks = (int) ((pouch.getTime() / 1000 / 30)  * (owner.getDonationManager().hasRank(DonatorRanks.DRAGON) ? 1.5 : 1));
		trackTimer = 0;
	}
	...
	public void call(boolean login) {
		...
		for (int dir = 0; dir < checkNearDirs[0].length; dir++) {
			final Position tile = new Position(new Position(owner.getX()
					+ checkNearDirs[0][dir], owner.getY()
					+ checkNearDirs[1][dir], owner.getZ()));
			if (World.canMoveNPC(tile.getZ(), tile.getX(), tile.getY(),
					size)) { // if found done
				teleTile = tile;
				break;
			}
	...
	public void drainSpecial(int specialReduction) {
		if (owner.getEquipment().wearingSkillCape(Skills.SUMMONING)) {
			specialReduction *= 0.70;
		}
	...
	public boolean hasSpecialOn() {
		if (owner.getTemporaryAttributtes().remove("FamiliarSpec") != null) {
			if (!owner.getInventory().containsItem(pouch.getScrollId(), 1) && !owner.getDonationManager().hasRank(DonatorRanks.DRAGON)) {
				owner.getPackets().sendGameMessage(
