...
import com.rs.game.world.entity.player.content.Pots;
...
public final class RefugeOfFear extends Controller {
	 ...
	 */
	private final Position[] spawnCoordinates = new Position[4];
	 ...
	 */
	public static final Position OUTSIDE_AREA = new Position(3149, 4664, 0);
	...
	public void start() {
		...
		if (!player.getDonationManager().hasRank(DonatorRanks.IRON)) {
			player.getDialogueManager().startDialogue("SimpleMessage", "You have to be an Iron Donator+ to enter this minigame.");
		...
		CoresManager.slowExecutor.schedule(new Runnable() {
			...
			public void run() {
				...
				player.setNextPosition(getWorldTile(13, 18));
				player.setForceMultiArea(true);
				//Spawn objects.
				Position base = getWorldTile(0, 0);
				/* North-east chamber */
	...
	public Position getWorldTile(int mapX, int mapY) {
		return new Position((boundChunks[0] << 3) + mapX, (boundChunks[1] << 3) + mapY, 0);
	}
	...
	public void spawnBosses() {
		Position[] coords = getSpawnCoordinates();
		int i = 0;
		for (Bosses monster : Bosses.values()) {
			NPC npc = new NPC(monster.getId(), coords[i++], -1, 0, false, false);
	 ...
	 */
	private Position[] getSpawnCoordinates() {
	 ...
	 * @return {@code True} if so.
	 */
	public static boolean isInRefugeOfFear(Position tile) {
	...
	private void endActivity(boolean logout) {
		...
		player.getControlerManager().removeControlerWithoutCheck();
		player.setNextPosition(OUTSIDE_AREA);
