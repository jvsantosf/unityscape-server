...
public final class ItemSpawns {
	...
	try {
		...
		Position tile = new Position(Integer.parseInt(splitedLine2[0]), Integer.parseInt(splitedLine2[1]), Integer.parseInt(splitedLine2[2]));
	...
	try {
		...
		World.addGroundItemForever(new Item(itemId, 1), new Position(x, y, plane));
	    }
	    channel.close();
	    in.close();
	}
    ...
    private static final void addItemSpawn(int itemId, int regionId, Position tile) {
	try {
