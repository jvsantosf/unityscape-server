...
import com.rs.game.world.entity.player.HintTrail;
...
public class WorldPacketsEncoder extends Encoder {
	...
	public void sendTestPacket(int interfaceId, int componentId, int unknownShort, int unknownInt) {
		OutputStream output = new OutputStream(10);
		output.writePacket(player, 127);
		output.writeInt(unknownInt);
		output.writeShort(unknownShort);
		output.writeInt(interfaceId << 16 | componentId);
		session.write(output);
	}

	public void setGe(Player player, int slot, int progress, int item, int price, int amount, int currentAmount) {
		OutputStream output = new OutputStream();
		output.writePacket(player, 53);
		output.writeByte(slot);
	...
	public void sendHintTrail(HintTrail trail) {
		OutputStream stream = new OutputStream(3);
		stream.writePacket(player, 149);
		stream.writeByte(trail.getIndex());
		stream.writeBigSmart(trail.getModelId());
		stream.writeSmart(trail.getSize() + 1);
		stream.writeShort(trail.getStart().getX());
		stream.writeShort(trail.getStart().getY());
		stream.writeByte(0);
		stream.writeByte(0);
		for (int i = trail.getSize() - 1; i >= 0; i--) {
			stream.writeByte(trail.getStepsX()[i] - trail.getStart().getX());
			stream.writeByte(trail.getStepsY()[i] - trail.getStart().getY());
		}
	}
	...
	public void sendProjectile(Entity receiver, Position startTile, Position endTile, int gfxId, int startHeight, int endHeight, int speed, int delay, int curve, int startDistanceOffset, int creatorSize) {
...
//	public void sendProjectile(Entity sender, Entity reciever, Projectile projectile) {
//		OutputStream stream = createWorldTileStream(sender);
//		stream.writePacket(player, 20);
//		int localX = sender.getLocalX(player.getLastLoadedMapRegionTile(), player.getMapSize());
//		int localY = sender.getLocalY(player.getLastLoadedMapRegionTile(), player.getMapSize());
//		int offsetX = localX - ((localX >> 3) << 3);
//		int offsetY = localY - ((localY >> 3) << 3);
//		stream.writeByte((offsetX << 3) | offsetY);
//		stream.writeByte(reciever.getX() - sender.getX());
//		stream.writeByte(reciever.getY() - sender.getY());
//		stream.writeShort(reciever == null ? 0 : (reciever instanceof Player ? -(reciever.getIndex() + 1) : reciever.getIndex() + 1));
...
//		stream.writeShort(projectile.getDuration(sender, reciever));
//		stream.writeByte(projectile.getAngle());
//		stream.writeShort(sender.getSize() * 64 + projectile.getDistanceOffset() * 64);
//		session.write(stream);
//	}
//
//	public void sendProjectile(Entity sender, Position reciever, Projectile projectile) {
//		OutputStream stream = createWorldTileStream(sender);
//		stream.writePacket(player, 20);
//		int localX = sender.getLocalX(player.getLastLoadedMapRegionTile(), player.getMapSize());
//		int localY = sender.getLocalY(player.getLastLoadedMapRegionTile(), player.getMapSize());
...
//
//
//	public void sendProjectile(Position sender, Entity reciever, Projectile projectile) {
//		OutputStream stream = createWorldTileStream(sender);
//		stream.writePacket(player, 20);
//		int localX = sender.getLocalX(player.getLastLoadedMapRegionTile(), player.getMapSize());
//		int localY = sender.getLocalY(player.getLastLoadedMapRegionTile(), player.getMapSize());
...
//		stream.writeShort(projectile.getDuration(sender, reciever));
//		stream.writeByte(projectile.getAngle());
//		stream.writeShort(1 * 64 + projectile.getDistanceOffset() * 64);
//		session.write(stream);
//	}
//
//	public void sendProjectile(Position sender, Position reciever, Projectile projectile) {
//		OutputStream stream = createWorldTileStream(sender);
//		stream.writePacket(player, 20);
//		int localX = sender.getLocalX(player.getLastLoadedMapRegionTile(), player.getMapSize());
//		int localY = sender.getLocalY(player.getLastLoadedMapRegionTile(), player.getMapSize());
...
//	}
	...
	public boolean sendConfigxP(int id, double exp, int value) {
		if (exp < Byte.MIN_VALUE || exp > Byte.MAX_VALUE)
	...
	public void sendIComponentText(int interfaceId, int componentId, String text) {
		...
		stream.writeInt(interfaceId << 16 | componentId);
		Huffman.sendEncryptMessage(stream, text);
		stream.endPacketVarShort();
		session.write(stream);
	...
	public void sendPresenceUpdate(RichPresenceType type, String value) {
		OutputStream stream = new OutputStream();
		switch (type) {
			case STATE -> stream.writePacket(player,161);
			case DETAILS -> stream.writePacket(player, 162);
			case LARGE_KEY -> stream.writePacket(player, 163);
			case LARGE_TEXT -> stream.writePacket(player, 164);
			case SMALL_KEY -> stream.writePacket(player, 165);
			case SMALL_TEXT -> stream.writePacket(player, 166);
		}
		Huffman.sendEncryptMessage(stream, value);
		session.write(stream);
	}
	...
	public void sendTileMessage(String message, Position tile, int color) {
		sendTileMessage(message, tile, 5000, 255, color);
	}

	public void sendTileMessage(String message, Position tile, int delay, int height, int color) {
	...
	public void receiveFriendChatMessage(String name, String display, int rights, String chatName,
			ChatMessage message) {
		OutputStream stream = new OutputStream();
	...
	public void sendDynamicMapRegion(boolean sendLswp) {
		...
		int mapHash = Constants.MAP_SIZES[player.getMapSize()] >> 4;
	...
	public void sendMapRegion(boolean sendLswp) {
		...
		for (int regionId : player.getMapRegionsIds()) {
			...
			if (xteas == null || Constants.isOSRSRegion(regionId))
				xteas = new int[4];
	...
	public void sendGraphics(Graphics graphics, Object target) {
		...
		if (target instanceof Player) {
			...
			Position tile = (Position) target;
			hash = tile.getZ() << 28 | tile.getX() << 14 | tile.getY() & 0x3fff | 1 << 30;
		}
	...
	public void sendDelayedGraphics(Graphics graphics, int delay, Position tile) {
	...
	public void sendGlobalString(int id, String string) {
		...
		if (string.length() > 253) {
			...
			stream.writeShort(id);
			Huffman.sendEncryptMessage(stream, string);
			stream.endPacketVarShort();
		} else {
	...
	public void sendMessage(int type, String text, Player p) {
		...
		stream.writeString(new String(Utils.getFormatedMessage(text)));
		Huffman.sendEncryptMessage(stream, text);
		stream.endPacketVarByte();
		session.write(stream);
	}
	...
	public void sendSkillLevel(int skill) {
		...
		stream.writeByte128(player.getSkills().virtualLeveling() ? player.getSkills().getVirtualLevel(skill) : player.getSkills().getLevel(skill));
		stream.writeByte(player.getSkills().virtualLeveling() ? 1 : 0);
		session.write(stream);
	}
