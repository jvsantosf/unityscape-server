...
public final class OutputStream extends Stream {
    ...
    public void checkCapacityPosition(int position) {
        if (position >= buffer.length) {
            byte[] newBuffer = new byte[16 + position];
    ...
    public void writeString(String s) {
        checkCapacityPosition(1 + offset + s.length());
        System.arraycopy(s.getBytes(), 0, buffer, offset, s.length());
        setOffset(s.length() + offset);
        writeByte(0);
    }
    ...
    public void endPacketVarByte() {
        writeByte(getOffset() - (opcodeStart + 1), opcodeStart);
    }
